...
type BranchQrCounterSignProps = {
  branchName: string;
  qrCodeUrl: string;
  publicLink: string;
};

export const COUNTER_SIGN_DIMENSIONS = {
  width: 900,
  height: 1200,
};

export const COUNTER_SIGN_LOGO_SRC = "/brand/enviosrd-logo.png";

export const COUNTER_SIGN_COPY = {
  title: "Escanea para pagar",
  subtitle: "Link de pago",
  branchLabel: "Sucursal",
  instruction: "Escanea el codigo y completa tu pago desde tu celular",
};

export const BranchQrCounterSign = React.forwardRef<
  HTMLDivElement,
  BranchQrCounterSignProps
>(function BranchQrCounterSign({ branchName, qrCodeUrl, publicLink }, ref) {
  return (
    <div
      ref={ref}
      ...
      <div className="space-y-5 px-5 py-6">
        <div className="text-center">
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#8a6806]">
            {COUNTER_SIGN_COPY.subtitle}
          </p>
          <h3 className="mt-2 text-[2.1rem] font-semibold tracking-tight text-[#0d2d4f]">
            {COUNTER_SIGN_COPY.title}
          </h3>
        </div>
        ...
        <div className="border-t border-dashed border-[#d6ded4] pt-4 text-center">
          ...
          <p className="mt-2 break-all text-xs leading-5 text-[#5a6f67]">{publicLink}</p>
        </div>
      </div>
    </div>
  );

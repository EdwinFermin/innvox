...
import type { CuadreFetchResult, CuadreSyncAssignment } from "@/types/cuadre.types";
...
export default function SyncCuadresPage() {
  ...
  const fetchMutation = useMutation({
    mutationFn: async () => {
      ...
      return fetchCuadre(branchId, date);
    },
    onSuccess: (data) => {
      setCuadre(data);
      setAssignments({});
  ...
  const applyMutation = useMutation({
    mutationFn: async () => {
      ...
      return applyCuadreSync({ branchId, date, assignments: transferAssignments });
    },
    onSuccess: (result) => {
      toast.success(

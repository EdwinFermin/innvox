-- 021_fix_apply_cuadre_sync_bank_tx_insert.sql
-- Fix a copy-paste bug introduced in migration 019: the INSERT into
-- bank_transactions had 9 target columns but only 8 expressions because
-- v_inserted_id (the value for linked_income_id) was accidentally dropped.
-- The result was `ERROR: INSERT has more target columns than expressions`
-- whenever applyCuadreSync was invoked.
--
-- This migration re-issues CREATE OR REPLACE with the corrected body and
-- re-applies the hardening from migration 020 (pinned search_path and
-- revoked public EXECUTE) so the function ends up in the intended state
-- regardless of the order in which a fresh database runs 019, 020, 021.
...
-- Re-apply hardening from migration 020 in case CREATE OR REPLACE reset
-- function-level grants (it does not, but this keeps 021 self-contained).
